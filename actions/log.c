...
extern void log_context_set_apppender(LogContext* context, void (*method)(const char*, size_t size)){
    if (!context || !method) {
...
extern void log_append(LogContext* context, const char* message, va_list format) {
    if (!context) {
...
extern void log_append_failover(LogContext* context, const char* message) {
    if (!context) {
        fprintf(stderr, "Log appender failed. Cog context is null. Failover called!\nMessage during failover: %s", message);
        return;
    }
    fprintf(stderr, "Log appender failed. Fallover called!\nMessage during failover: %s", message);
    return;
}
