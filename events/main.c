...
static void register_log_area_events(MainUi * main_ui) {
    g_signal_connect(main_ui->log_text_view, "size-allocate", G_CALLBACK(event_log_auto_scroll), gpointer_ui_with_data(main_ui, NULL));
}

static void register_menu_bar_top_events(MainUi * main_ui) {
    g_signal_connect(main_ui->menu_bar_items.load_file, "activate", G_CALLBACK(event_menu_bar_load), gpointer_ui_with_data(main_ui, NULL));
}
static void register_code_area_events(MainUi * main_ui) {
    g_signal_connect(main_ui->code_text_buffer, "end-user-action", G_CALLBACK(event_code_buffer_change),NULL);

}
static void register_opengl_area_events(MainUi * main_ui) {
    g_signal_connect(main_ui->opengl_area, "realize", G_CALLBACK(display_setup),NULL);

    g_signal_connect(main_ui->opengl_area, "render", G_CALLBACK(display_render),NULL);
...
static void event_menu_bar_load(GtkMenuItem* item, gpointer data) {
    ...
    GtkWidget *dialog = gtk_file_chooser_dialog_new ("Open File", GTK_WINDOW(event.references->window), action, "_Cancel", 
    ...
    if (res == GTK_RESPONSE_ACCEPT) {
        ...
        if ( file_contents != NULL ) {
            gtk_text_buffer_set_text(event.references->code_text_buffer, file_contents, -1);
            free((void*)file_contents);
        }
        g_signal_emit_by_name((gpointer)event.references->menu_bar_items.load_file, "end-user-action");
        free ((void*)filename);
    }
...
static void event_log_auto_scroll(GtkTextView* log, gpointer data) {
    GtkAdjustment* vertical = gtk_scrollable_get_vadjustment(GTK_SCROLLABLE(log));
    gdouble value = gtk_adjustment_get_upper(vertical) - gtk_adjustment_get_page_size(vertical);
